    ...
    for(j in 1:(num_conditions-1))
    {
      for(k in (j+1):num_conditions)
      {
        if(out[[j]]$mark==1 && out[[k]]$mark==1)
        {
          ...
          compare_j_k = likelihood_ratio_tissue_generalized_poisson(y[j,],out[[j]]$lambda,out[[j]]$theta,y[k,],out[[k]]$lambda,out[[k]]$theta,norm);
          if(do_permute == 0)
          {
          ...
          {
            gamma = perform_permutation_de(y[j,],y[k,],1000);
          }
