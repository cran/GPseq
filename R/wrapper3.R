estimate_differential_splicing<-function(reads,exons,genes,do_permute)
{
  ...
  for(i in 1:num_genes)
  {
    ...
    out_gene = apply(y,1,generalized_poisson_likelihood);
    ...
    for(e in genes[i,2]:genes[i,3])
    {
      ...
      out_exon = apply(z,1,generalized_poisson_likelihood);
      ...
      for(r in 1:(num_conditions-1))
      {
        for(tr in (r+1):num_conditions)
        {
          ...
          {
            out_lr = likelihood_ratio_generalized_poisson_exon_gene(z[r,],out_exon[[r]]$theta,out_exon[[r]]$lambda,y[r,],out_gene[[r]]$theta,out_gene[[r]]$lambda,z[tr,],out_exon[[tr]]$theta,out_exon[[tr]]$lambda,y[tr,],out_gene[[tr]]$theta,out_gene[[tr]]$lambda);
            out_lrp = likelihood_ratio_poisson_exon_gene(z[r,],y[r,],z[tr,],y[tr,]);
            gamma = list(shape = -1,scale = -1);
            if(out_lr$mark == 1 && do_permute == 1)
            {
              gamma = perform_permutation_ds(y[r,],y[tr,],exon_start_map[exon_id],exon_end_map[exon_id],n,1000);
